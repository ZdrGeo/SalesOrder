...
(function(factory){
  ...
  Sammy = function() {
    ...
    if (args.length === 0 || args0] && _isFunction(args0])) { // Sammy()
      return Sammy.apply(Sammy, 'body'].concat(args));
    } else if (typeof (selector = args.shift()) == 'string') { // Sammy('#main')
      app = Sammy.appsselector] || new Sammy.Application();
      ...
      if (app.element_selector != selector) {
        delete Sammy.appsselector];
      }
      Sammy.appsapp.element_selector] = app;
      return app;
    }
  };
  ...
  $.extend(Sammy.Object.prototype, {
    ...
    keys: function(attributes_only) {
      ...
      for (var property in this) {
        if (!_isFunction(thisproperty]) || !attributes_only) {
          keys.push(property);
        }
      }
      return keys;
    ...
    has: function(key) {
      return thiskey] && $.trim(thiskey].toString()) !== '';
    },
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    route: function(verb, path) {
      ...
      add_route = function(with_verb) {
        ...
        app.routeswith_verb] = app.routeswith_verb] || ];
        // place routes in order of definition
        app.routeswith_verb].push(r);
      };
    ...
    bind: function(name, data, callback) {
      ...
      if (!this.listenersname]) { this.listenersname] = ]; }
      this.listenersname].push(listener_callback);
      if (this.isRunning()) {
    ...
    helper: function(name, method) {
      this.context_prototype.prototypename] = method;
      return this;
    },
    ...
    destroy: function() {
      this.unload();
      delete Sammy.appsthis.element_selector];
      return this;
    },
    ...
    lookupRoute: function(verb, path) {
      ...
      if (typeof this.routesverb] != 'undefined') {
        l = this.routesverb].length;
        for (; i < l; i++) {
          route = this.routesverb]i];
      ...
      if (route) {
        ...
        if ((path_params = route.path.exec(this.routablePath(path))) !== null) {
          ...
          $.each(path_params, function(i, param) {
            ...
            if (route.param_namesi]) {
              // set the name to the match
              paramsroute.param_namesi]] = _decode(param);
            } else {
              // initialize 'splat'
        ...
        wrapped_route = function() {
          ...
          while (befores.length > 0) {
            ...
            // check the options
            if (app.contextMatchesOptions(context, before0])) {
              returned = before1].apply(context, context]);
          ...
          if (route.callback && route.callback.length) {
            ...
            nextRoute = function() {
              i++;
              if (route.callbacki]) {
                returned = route.callbacki].apply(context,callback_args);
    ...
    contextMatchesOptions: function(context, match_options, positive) {
      ...
      if (_isArray(options.path)){
        ...
        for (numopt = 0, len = options.path.length; numopt < len; numopt += 1) {
          opts = $.extend({}, options, {path: options.pathnumopt]});
    ...
    templateCache: function(key, value) {
      if (typeof value != 'undefined') {
        return _template_cachekey] = value;
      } else {
        return _template_cachekey];
      }
    },
    ...
    _checkLocation: function() {
      ...
      if (!this.last_location || this.last_location0] != 'get' || this.last_location1] != location) {
        // reset last location
    ...
    _getFormVerb: function(form) {
      ...
      $_method = $form.find('inputname="_method"]');
      if ($_method.length > 0) { verb = $_method.val(); }
      if (!verb) { verb = $form0].getAttribute('method'); }
    ...
    _serializeFormParams: function($form) {
       var queryString = "",
         ...
         i;
       if (fields.length > 0) {
         queryString = this._encodeFormPair(fields0].name, fields0].value);
         for (i = 1; i < fields.length; i++) {
           queryString = queryString + "&" + this._encodeFormPair(fieldsi].name, fieldsi].value);
         }
       }
       return queryString;
    },
    ...
    _parseFormParams: function($form) {
      ...
      for (i = 0; i < form_fields.length; i++) {
        params = this._parseParamPair(params, form_fieldsi].name, form_fieldsi].value);
      }
      return params;
    },
    ...
    _parseQueryString: function(path) {
      ...
      if (parts && parts1]) {
        ...
        for (i = 0; i < pairs.length; i++) {
          pair = pairsi].split('=');
          params = this._parseParamPair(params, _decode(pair0]), _decode(pair1] || ""));
        }
      }
      return params;
    },
    ...
    _parseParamPair: function(params, key, value) {
      if (typeof paramskey] !== 'undefined') {
        if (_isArray(paramskey])) {
          paramskey].push(value);
        } else {
          paramskey] = paramskey], value];
        }
      } else {
        paramskey] = value;
      }
    ...
    then: function(callback) {
      if (!_isFunction(callback)) {
        ...
        if (typeof callback === 'string' && callback in this.event_context) {
          var helper = this.event_contextcallback];
        ...
        if (typeof location === 'string') {
          ...
          if (location.selector) {
            ...
            if (options.clone === false) {
              ...
              return location0].innerHTML.toString();
            }
          }
        }
      });
      ...
      if(partials) {
        ...
        for(name in partials) {
          (function(context, name) {
            context.load(partialsname])
                   .then(function(template) {
                     this.partialsname] = template;
                   });
          })(this, name);
        }
      }
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    engineFor: function(engine) {
      ...
      if ((engine_match = engine.match(/\.(^\.\?\#]+)(\?|$)/))) {
        engine = engine_match1];
      }
      // set the engine to the default template engine if no match is found
      if (engine && _isFunction(contextengine])) {
    ...
    redirect: function() {
      ...
      if (l > 1) {
        ...
        if (has_params) {
          for (var k in params) {
            pairs.push(this.app._encodeFormPair(k, paramsk]));
          }
          to += '?' + pairs.join('&');
        }
      } else {
